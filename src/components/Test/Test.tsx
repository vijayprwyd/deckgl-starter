...
import sampleGeoJson from "../sample.json";

import iconSheet from "./edit-handles.png";
...
import { getInitialViewState } from "../FloorCanvas/FloorCanvasUtils";
...
> {
    ...
    return (
      <div key={index}>
        <ToolboxCheckbox
          ...
          onChange={() => {
            if (selectedFeatureIndexes.includes(index)) {
              this.setState({
                selectedFeatureIndexes: selectedFeatureIndexes.filter(
                  (e) => e !== index
                ),
              });
            } else {
        ...
        >
          <span
            style={{
              color: this._getHtmlColorForFeature(
                index,
  ...
  _renderSelectFeatureCheckboxes() {
    ...
    for (let i = 0; i < features.length; ++i) {
      checkboxes.push(
        this._renderSelectFeatureCheckbox(i, features[i].geometry.type)
      );
    }
    return checkboxes;
  }
          ...
          {operations.map((operation) => (
            <ToolboxButton
              ...
              selected={
                ...
                this.state.modeConfig.booleanOperation === operation
              }
              onClick={() => {
                if (
                  this.state.modeConfig &&
      ...
      <ToolboxRow key="twoClick">
        ...
        <ToolboxControl>
          <input
            type="checkbox"
            checked={Boolean(
              this.state.modeConfig && this.state.modeConfig.dragToDraw
            )}
            onChange={(event) =>
              this.setState({
                modeConfig: {
        ...
        <ToolboxRow>
          ...
          <ToolboxControl>
            <input
              type="checkbox"
              checked={Boolean(
                this.state.modeConfig && this.state.modeConfig.enableSnapping
              )}
              onChange={(event) => {
                const modeConfig = {
                  ...this.state.modeConfig,
      ...
      <ToolboxRow key="measure-distance">
        ...
        <ToolboxControl>
          <input
            ...
            checked={Boolean(
              this.state.modeConfig &&
                this.state.modeConfig.centerTooltipsOnLine
            )}
            onChange={(event) => {
              const modeConfig = {
                ...this.state.modeConfig,
      ...
      <ToolboxRow key="draw-polygon">
        ...
        <ToolboxControl>
          <input
            ...
            checked={Boolean(
              this.state.modeConfig &&
                this.state.modeConfig.preventOverlappingLines
            )}
            onChange={(event) =>
              this.setState({
                modeConfig: {
    ...
    return (
      <Toolbox>
          ...
          <React.Fragment>
            ...
            <ToolboxControl>
              <textarea
                ...
                onChange={(event) =>
                  this.setState({
                    testFeatures: JSON.parse(event.target.value),
                  })
                }
              />
            </ToolboxControl>
        ...
        <ToolboxRow>
          ...
          <ToolboxControl>
            <ToolboxButton onClick={() => this._loadSample("mixed")}>
              Mixed Sample
            </ToolboxButton>
            <ToolboxButton onClick={() => this._loadSample("complex")}>
              Complex Sample
            </ToolboxButton>
            <ToolboxButton onClick={() => this._loadSample("blank")}>
              Blank
            </ToolboxButton>
            <ToolboxButton onClick={() => this._loadSample("file")}>
              Open file...
            </ToolboxButton>
          </ToolboxControl>
        </ToolboxRow>
          ...
          <ToolboxControl>
            <ToolboxCheckbox
              ...
              onChange={() =>
                this.setState({
                  editHandleType:
                    this.state.editHandleType === "icon" ? "point" : "icon",
                })
              }
            >
              Use Icons
  ...
  _renderFeatureMenu({ x, y }: { x: number; y: number }) {
    return (
      <div style={{ position: "fixed", top: y - 40, left: x + 20 }}>
        <ToolboxButton onClick={() => this._featureMenuClick("delete")}>
          Delete
        </ToolboxButton>
        <ToolboxButton onClick={() => this._featureMenuClick("split")}>
          Split
        </ToolboxButton>
        <ToolboxButton onClick={() => this._featureMenuClick("info")}>
          Info
        </ToolboxButton>
        <ToolboxButton onClick={() => this._featureMenuClick("")}>
          Close
        </ToolboxButton>
      </div>
    );
    ...
    return (
      <div className="relative grow">
        ...
        {this.state.featureMenu &&
          this._renderFeatureMenu(this.state.featureMenu)}
      </div>
    );
  }
}
